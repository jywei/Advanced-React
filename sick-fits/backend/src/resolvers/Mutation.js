...
const Mutations = {
  async createItem(parent, args, ctx, info) {
    ...
    const item = await ctx.db.mutation.createItem({
      data: {
        // This is how we create an association between the Item and the User
        user: {
          connect: {
  ...
  async addToCart(parent, args, ctx, info) {
    ...
    // 3. Check if that item is already in their cart and
  ...
  async removeFromCart(parent, args, ctx, info) {
    ...
    // 2. make dure they own that cart item
    if (cartItem.user.id !== ctx.request.userId) {
      throw new Error('Cheating huhhhh?');
    };
    // 3. delete that cart tiem
